...
import { singInWithEmail } from '@/db/auth'
...
export default function login () {
  ...
  const handleSubmit = async (event) => {
    ...
    const { session, user } = await singInWithEmail({ email, password })
  ...
  return (
    <Flex as='main' direction='column' gap={5}>
      ...
      <Heading as='h1'>Sing in to your account</Heading>
      ...
      <Flex onSubmit={handleSubmit} as='form' direction='column' gap={3}>
        ...
        <Button type='submit' colorScheme='whatsapp'>Sign In</Button>
      </Flex>
